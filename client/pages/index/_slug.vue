<template>
  <div class="flex-1 flex flex-col h-screen text-brown">
    <!--
    <div class="relative">
      <div class="flex items-center justify-end px-5 py-4 border-b h-16">
        <div class="flex items-center">
          ...
          <button v-if="isBookmarkAvailable" @click="handleBookmarkAction" class="ml-3">
          ...
          <button @click="toPreviousRecipe" class="ml-3" >
            <arrow-left-circle :color="$store.getters.getPreviousRecipeInList(slug) ? '#00d1b2' : '#555555'"/>
          </button>
          <button @click="toNextRecipe" class="ml-3">
            <arrow-right-circle :color="$store.getters.getNextRecipeInList(slug) ? '#00d1b2' : '#555555'" />
          </button>
        </div>

        <div class="flex items-center">
          <button @click="toPreviousRecipe" >
            <arrow-left-circle :color="$store.getters.getPreviousRecipeInList(slug) ? '#00d1b2' : '#555555'"/>
          </button>
          <button @click="toNextRecipe">
            <arrow-right-circle :color="$store.getters.getNextRecipeInList(slug) ? '#00d1b2' : '#555555'" />
          </button>
        </div>
...
--->
    <div class="p-3 flex-1 overflow-y-auto">
      <div class="recipe-header flex px-3">
        <div class="flex flex-col">
          ...
          -->
          <h1 class="mt-6 font-bold text-4xl">
            {{ title | truncate(36) }}
          </h1>
          <p class="text-xs text-brown">Publiée {{ publishedAt }}, par @{{ author.username }}</p>
        </div>
        
      </div>
      <div class="recipe-wrapper flex w-full my-6">
        <div class="left-side flex flex-col">
          ...
          <div class="mr-3 p-3 border-t">
            <div class="flex items-end">
              <claps-component :total-count="getTotalInteractionsCount" :user-interaction-count="isUserInteractionCountAvailable" @handleClick="sendLove" />
              <!-- TODO Remplacer cette icone par une etoile -->
              <button v-if="isBookmarkAvailable" @click="handleBookmarkAction" class="ml-3">
                <bookmark-icon-component :is-bookmarked="isBookmarked" height="h-7" width="w-7" />
              </button>
              <button class="ml-5">
...
export default {
  components: { BookmarkIconComponent, ClapsComponent, ArrowLeftCircle, ArrowRightCircle },
  validate ({ params }) {
  ...
  methods: {
    ...
    toNextRecipe () {
      console.log(this.$store.getters.getNextRecipeInList(this.slug))
